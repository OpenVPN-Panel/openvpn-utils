...
: "${HOST_IP_DOMAIN:?Need to set HOST_IP_DOMAIN (your public IP or domain)}"
PUBLIC_IP=${HOST_IP_DOMAIN}
...
# === [1/2] Preparing a Public Key Infrastructure Directory ==
...
# === [2/2] Creating a Certificate Authority ===
...
cp $EASYRSA_SERVER_DIR/pki/private/$VPN_SERVER_CN.key $SERVER_CONF_DIR

# === [4/10] Signing the OpenVPN Servers Certificate Request ===

cp $EASYRSA_SERVER_DIR/pki/reqs/$VPN_SERVER_CN.req $OPENVPN_TMP_DIR
cd $EASYRSA_DIR
./easyrsa import-req $OPENVPN_TMP_DIR/$VPN_SERVER_CN.req $VPN_SERVER_CN
./easyrsa --batch sign-req server $VPN_SERVER_CN

cp pki/issued/$VPN_SERVER_CN.crt $OPENVPN_TMP_DIR
cp pki/ca.crt $OPENVPN_TMP_DIR

cp $OPENVPN_TMP_DIR/{$VPN_SERVER_CN.crt,ca.crt} $SERVER_CONF_DIR

# === [5/10] Configuring OpenVPN Cryptographic Material ===

cd $EASYRSA_SERVER_DIR
openvpn --genkey --secret ta.key
cp ta.key $SERVER_CONF_DIR
...
cp pki/reqs/$CLIENT_NAME.req $OPENVPN_TMP_DIR

cd $EASYRSA_DIR
./easyrsa import-req $OPENVPN_TMP_DIR/$CLIENT_NAME.req $CLIENT_NAME
./easyrsa --batch sign-req client $CLIENT_NAME

cp pki/issued/$CLIENT_NAME.crt $OPENVPN_TMP_DIR

cp $OPENVPN_TMP_DIR/$CLIENT_NAME.crt $CLIENT_CONFIGS_DIR/keys/

cp $EASYRSA_SERVER_DIR/ta.key $CLIENT_CONFIGS_DIR/keys/
cp $SERVER_CONF_DIR/ca.crt $CLIENT_CONFIGS_DIR/keys/

# === [7/10] Configuring OpenVPN ===
cp /tmp/server.conf $SERVER_CONF_DIR

# === [8/10] Creating the Client Configuration Infrastructure ===
mkdir -p $CLIENT_CONFIGS_DIR/files || true
cp /tmp/base.conf $CLIENT_CONFIGS_DIR/base.conf
